...
- The API User can use the data provided for commerical purposes.
...
- Once The API User's subscription ends, The API User can still keep the data retrieved from the API, however the attribution clause still aplies. Images cannot be used after the end of the subscription.

- Allowed monthly bandwidth for the images per subscription level are: 1GB for Free Plan, 10GB For Pro Plan, 25GB for Ultra Plan, and 100GB for Mega Plan. The API User cannot use the images in a way that would exceed the given bandwidth limits.
